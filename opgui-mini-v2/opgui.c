...
#include "fileIO.h"
#include "icons.h"
#include "icons2.h"//prueba de iconos random
#include "progAVR.h"
#define MAXLINES 600
...
void TestHw();
int CheckS1();
char *filename;//pongo global para usar Load en cualquier parte
...
void Fopen(GtkWidget *widget,GtkWidget *window)
{
    ...
    if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT){
	    //char *filename;
		g_free (filename);//cambio de sitio para que quede almacenado filename
...
void VPPVDDactive(GtkWidget *widget,GtkWidget *window)
{
	...
	gtk_statusbar_push(GTK_STATUSBAR(status_bar),statusID,str);//barra de estado informacion
...
int main( int argc, char *argv[])
{
	...
	sprintf(str,"opgui mini v%s",VERSION);
...
//------toolbar-------------
	...
	//fix de botones invisibles
    gtk_toolbar_insert_stock(GTK_TOOLBAR(toolbar),GTK_STOCK_GO_UP,strings[I_DevR],"",G_CALLBACK(DevRead),0,-1);//"Leer"
	gtk_toolbar_insert_stock(GTK_TOOLBAR(toolbar),GTK_STOCK_JUMP_TO,strings[I_DevW],"",G_CALLBACK(DevWrite),0,-1);//"Escribir"
	...
	//gtk_notebook_append_page(GTK_NOTEBOOK(notebook),GTK_TOOLBAR(toolbar),toolbar);
	GtkWidget * devHbox1 = gtk_hbox_new(FALSE,10);
	//gtk_table_attach(GTK_TABLE(table_dev),devHbox1,0,2,0,1,4,0,5,5);
	gtk_toolbar_insert_widget(GTK_TOOLBAR(toolbar),devHbox1, "","",-1);
...
//------status bar-------------
	...
	//*** Desactivo agunos de los tabs 
	//gtk_notebook_remove_page(GTK_NOTEBOOK(notebook),2);//Borro el tab opciones
	//gtk_notebook_remove_page(GTK_NOTEBOOK(notebook),3);//Borro el tab opciones
	//gtk_notebook_remove_page(GTK_NOTEBOOK(notebook),4);//Borro el tab opciones
	//gtk_notebook_remove_page(GTK_NOTEBOOK(notebook),5);//Borro el tab opciones
	//gtk_notebook_remove_page(GTK_NOTEBOOK(notebook),6);//Borro el tab opciones
	//gtk_widget_destroy(optTable);
	//gtk_widget_destroy(i2cTable);
	//gtk_widget_destroy(icdVbox1);
	//gtk_widget_destroy(ioVbox1);
	//gtk_widget_destroy(utVbox1);
	 gtk_notebook_set_show_tabs (notebook,FALSE);//escondo todos los TABS
...
int StartHVReg(double V){
	...
	bufferU[j++]=READ_ADC;
	bufferU[j++]=FLUSH;
	//me aseguro que empieze en bajo siempre
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (VPP_ON), FALSE);
	for(;j<DIMBUF;j++) bufferU[j]=0x0;
	PacketIO(5);
	msDelay(20);
...
//	PrintMessage2("v=%d=%fV\n",v,v/G);
	if(v==0){
		//enciendo vppu un instante pata descargar el inductor
	     gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (VPP_ON), TRUE);
...
//		PrintMessage2("v=%d=%fV\n",v,v/G);
	...
	//posible arreglo problema de tierra
	if(v>(vreg/10.0+1)*G){
		//enciendo vppu un instante pata descargar el inductor
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (VPP_ON), TRUE);
		/* if(v>(vreg/10.0+1)*G){
...
/*	if(info){
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(b_V33check),TRUE);
